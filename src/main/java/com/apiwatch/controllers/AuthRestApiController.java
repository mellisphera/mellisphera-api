...
public class AuthRestApiController {
	...
	public ResponseEntity<?> authenticateUser(@Valid @RequestBody LoginForm loginRequest) {
		log.info(" Sign Up : username :" + loginRequest.getUsername() + " password:" + loginRequest.getPassword());

		Authentication authentication = authenticationManager.authenticate(
				new UsernamePasswordAuthenticationToken(loginRequest.getUsername(),loginRequest.getPassword()));
		log.info(" Sign Up : AUTH OK");
	...
	public ResponseEntity<?> registerUser(@Valid @RequestBody User signUpRequest) {
		log.info(" Sign Up : username :"+ signUpRequest.getLogin().getUsername() +" password :"+signUpRequest.getPassword()+" email:" + signUpRequest.getEmail());
		//
		Login credential = new Login(signUpRequest.getUsername(),encoder.encode(signUpRequest.getPassword()));
		//
		log.info(" Sign Up : Login :"+ credential.getUsername() +" password :" + credential.getPassword());
		//Search if user already exit
		if (userRepository.findUserByLogin(credential) != null) {
			log.error(" Sign Up : Fail -> Username is already taken! "+ signUpRequest.getLogin().getUsername() );
	...
	**/
		...
		User user = new User(signUpRequest.getId(), GregorianCalendar.getInstance().getTime(),credential,signUpRequest.getPhone(),signUpRequest.getEmail(),0,null,signUpRequest.getFullName(),signUpRequest.getPosition(),signUpRequest.getCity(), signUpRequest.getCountry(),signUpRequest.getRoles());
